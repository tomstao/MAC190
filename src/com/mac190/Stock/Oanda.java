...
public class Oanda extends Broker implements iBroker {
    ...
    public void buy(String t, String type, int s, float p) {
        //if type is not "cur" or "cur", then throw Exception "Can only buy currencies"
    ...
    public double sell(String t, String type, int s, float p) {
        //if type is not "cur" throw and exception wrong type
        if(!type.equalsIgnoreCase("cur")) {
            throw new IllegalArgumentException("Can only sell Currencies!");
        }
        ...
        se.setShares(se.getShares() - s);
        System.out.println("You successfully sold currencies total of $" + total);
